...
function updateUser(id, name) {
    if(name.length > 2 && name.length < 17 && name.indexOf(' ') < 0 && !utils.checkUser(clients, name) && name.match(alphanumeric) && name != 'Console' && name != 'System') {
        ...
        if(name == 'Admin')
        {
            console.log("in connection -----> "+ name);
            adminSetRole(name);
        }
...
function adminSetRole(name)
{
            var string = 'Console gave ' + name + ' administrator permissions';
            var data = {};
            data.message = string;
            data.user = 'Console';
            data.type = 'role';
            ...
            utils.sendToAll(clients, data);
            utils.sendToOne(clients, users, data, name, data.type);
}
...
if(!config.ssl.use) {
    ...
    //yeh bhi ignore karo, yeh wala lagega jab production deploy hoga.
