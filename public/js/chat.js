        ...
        if(data.type == 'server') {
            switch(data.info) {
                ...
                case 'success':
                    ...
                    $('#current_user_name').text(username);
                ...
                case 'user':
                    ...
                    console.log(user_data.un +"user rolee-------------");
                    break;
            }
...
function handleInput() {
    ...
    console.log("Message -----> "+"/pm "+$('#setPmUserName').text()+" "+value);
    if( $('#setPmUserName').text() === 'Welcome to BorisChat')
    {
        value = value;
        $('#setPmUserName').text('Chat');
        
    } else if($('#setPmUserName').text() === 'Chat') {
        value =  "/pm Admin "+value;
    }else {
        value = "/pm "+$('#setPmUserName').text()+" "+value;
    }
    if(value.length > 0) {
...
$(document).ready(function() {
    $('#user').bind('click', function() {
        ...
        for(var i in clients) {
            if(clients[i] != undefined) {
                ...
                if(clients[i].role === 1) {
                    admin = ' - <b>Helper</b></button></li>';
                }

                if(clients[i].role === 2) {
                    admin = ' - <b>Moderator</b></button></li>';
                }

                if(clients[i].role === 3) {
                    admin = ' - <b>Administrator</b></button></li>';
                }

                console.log(clients[i].un);
                username_item = clients[i].un;
                content += '<li><button style="width:100%; height: 100px;" onclick="setPmUser(\''+username_item+'\')" >' + '<b>#' + clients[i].id + '</b> ' + userip + ' - ' + clients[i].un + admin;
            }
        }
    ...
    .autocomplete({
        ...
        select: function(event, ui) {
            ...
            var old = terms.pop();  
            var ins = "@" + ui.item.value + " "; 
            var ind = this.value.lastIndexOf(old); 
...
//ignore karo isko, yeh sab mein personal project ke liye add kiya hai, module abhi bhi incomplete hai.
//these wont affect other things....
...
function setPmUser(username_item) {
    console.log("user name"+username_item);
    $('#setPmUserName').html(username_item);
